...
# --- NOUVELLE FONCTION : RESTAURATION DES DONNÉES DEPUIS LA SAUVEGARDE ---
def restaurer_donnees_depuis_sauvegarde():
    """
    Restaure toutes les données (agents, planning, heures, cloud_data, tâches, etc.)
    ...
    try:
        ...
        # Vider les tables concernées (on garde les utilisateurs)
        conn = db_manager.get_db()
        c = conn.cursor()
        ...
        # --- 5. Tâches (terminées et en cours) ---
        # On va utiliser les données de taches_operateur et taches_en_cours
        # Mais on ne peut pas les restaurer directement car les IDs d'agents peuvent avoir changé.
        # On va attribuer les tâches à l'agent qui correspond au nom de l'utilisateur de la sauvegarde (ou à un agent générique)
        # On peut aussi les ignorer et se contenter de restaurer l'historique des points.
        # Pour une restauration complète, on pourrait les réinjecter avec les noms.
        # Mais ici, on va simplement les restaurer dans la session (st.session_state) et non en DB.
        # On restaure les variables de session.
        ...
        # Pour les couleurs, on les restaure aussi
...
with st.sidebar:
    ...
    if st.session_state.user_role == "admin":
        ...
        # --- NOUVEAU BOUTON : RESTAURER DEPUIS LA SAUVEGARDE ---
